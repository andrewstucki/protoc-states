...
func NewLibSQLBackendBuilder() *LibSQLBackendBuilder {
	workerId := generateWorkerId()

	return &LibSQLBackendBuilder{
		workerName:               workerId,
		host:                     "localhost:8080",
		scheme:                   "http",
...
func (b *LibSQLBackendBuilder) WithScheme(scheme string) *LibSQLBackendBuilder {
	b.scheme = scheme
	return b
}

func (b *LibSQLBackendBuilder) WithHost(host string) *LibSQLBackendBuilder {
	b.host = host
	return b
}

func (b *LibSQLBackendBuilder) WithLocalReplica(replica string) *LibSQLBackendBuilder {
	b.localReplica = replica
	return b
}

func (b *LibSQLBackendBuilder) WithOrchestrationLockTimeout(timeout time.Duration) *LibSQLBackendBuilder {
	b.orchestrationLockTimeout = timeout
	return b
}

func (b *LibSQLBackendBuilder) WithActivityLockTimeout(timeout time.Duration) *LibSQLBackendBuilder {
	b.activityLockTimeout = timeout
	return b
}

func (b *LibSQLBackendBuilder) WithToken(token string) *LibSQLBackendBuilder {
	b.token = token
	return b
}
...
func (b *LibSQLBackendBuilder) build(logger backend.Logger) *libsqlBackend {
	...
	if b.token != "" {
		dbUrl = fmt.Sprintf("%s?authToken=%s", dbUrl, b.token)
	}
	be.dsn = dbUrl
...
func (be *libsqlBackend) initDB() error {
	...
	if be.localReplica == "" {
		...
		if be.token != "" {
			opts = append(opts, libsql.WithAuthToken(be.token))
		}
...
func (be *libsqlBackend) Stop(context.Context) error {
	if err := be.db.Close(); err != nil {
		return err
	}
	if be.connector != nil {
		return be.connector.Close()
	}
	return nil
}
